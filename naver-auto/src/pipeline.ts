...
export async function publishDraft(post: Post, settings: Settings, report: Reporter): Promise<Post> {
  ...
  // Capture after the guard: narrowing does not survive into the closure below.
  const blogId = account.blogId;
  ...
  return withProfile("publish", async () => {
    let context: BrowserContext | null = null;
    try {
      context = await naverContext(!settings.headful);
      const page = context.pages()[0] ?? (await context.newPage());

      const frame = await openEditor(page, blogId, report);
      post.status = "editing";
      upsertPost(post);
